...
def plot_bar_chart( MDF,g3,g4,r_id,shp_r, ax = None):
    ...
    ax.bar(reaction_id, dG_phy, color = 'royalblue', label='Uniform concentrations (1 mM)')
    ...
    ax.set_title('MDF = {:0.2f} kJ/mol'.format(MDF),fontsize=11)
...
def plot_driving_forces_my(mdf_result):
        ...
        ax.plot(
            yvalues_phy,
            label="Uniform concentrations (1 mM)",
            color="lightgreen",
            linestyle="--",
            zorder=1,
        )

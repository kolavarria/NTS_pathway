...
def translate_and_filter_reaction(rxn, cmpd_dict, threshold=1e-5):
    """Handles PhasedReaction objects by parsing their string representation."""
    def parse_side(side_str):
        terms = []
        for term in side_str.split(" + "):
            parts = term.strip().split(" ", 1)
            if len(parts) == 1:
                coeff, compound = 1.0, parts[0]  # Implicit coefficient of 1.0
            else:
                coeff_str, compound = parts
                coeff = abs(float(coeff_str))  # Use absolute value for parsing
            if coeff >= threshold:
                terms.append(term.strip())
        return " + ".join(terms)

    # Get reaction string via existing translation
    translated_rxn = translate_reaction(rxn, cmpd_dict)
    
    # Split into left/right and filter
    left_str, right_str = translated_rxn.split(" <=> ")
    return f"{parse_side(left_str)} <=> {parse_side(right_str)}"


# ---------- MODIFIED MDF_from_pathway ----------
def MDF_from_pathway(pathway, cmpd_settings, print_results=True):
    mdf_result = pathway.mdf_analysis()
    mdf_result.reaction_df["reaction_formula"] = [
        translate_reaction(rxn, cmpd_settings.cmpstrs2acr_dict) 
        for rxn in mdf_result.reaction_df["reaction_formula"]
    ]

    if not pathway.net_reaction.is_balanced():
        print("Net reaction is not balanced!")

    mdf_result.net_reaction = pathway.net_reaction

    if print_results:
        print_MDF_results(mdf_result, cmpd_settings)  # Pass settings for translation
...
# ---------- MODIFIED print_MDF_results ----------
def print_MDF_results(mdf_result, cmpd_settings=None):
    if cmpd_settings:
        # Translate reaction formulae
        mdf_result.reaction_df["reaction_formula"] = [
            translate_reaction(rxn, cmpd_settings.cmpstrs2acr_dict)
            for rxn in mdf_result.reaction_df["reaction_formula"]
        ]
    ...
    # Display results
    display(f"MDF = {mdf_result.score:.2f} kJ/mol")
    
    if cmpd_settings:
        clean_net_rxn = translate_and_filter_reaction(
            mdf_result.net_reaction,
            cmpd_settings.cmpstrs2acr_dict,
            threshold=1e-5  # Adjust as needed
        )
        display(f"Net Reaction: {clean_net_rxn}")
    
    display(mdf_result.reaction_df)
    display(mdf_result.compound_df)

    try:
        mdf_result.plot_concentrations().show()
        mdf_result.plot_driving_forces().show()
...
def MDF_CVA(MDF, pathway_file_name,cmpd_settings, physiological_context, compounds = None, custom_dGs = True, print_results = True, y=0):
	if type(MDF) is Q_:
		MDF = MDF.m_as('kJ/mol')
